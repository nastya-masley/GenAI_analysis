...
const form = document.getElementById('proc-analyze-form');
const statusEl = document.getElementById('proc-status');
const resultSection = document.getElementById('proc-result');
const resultText = document.getElementById('proc-result-text');
const submitBtn = document.getElementById('proc-submit-btn');
const showAnalyticsBtn = document.getElementById('proc-show-analytics-btn');
const outputsPanel = document.getElementById('proc-analytics-bottom');
const previewEl = document.getElementById('proc-preview');
const videoPlaceholder = document.getElementById('proc-video-placeholder');
const canvasPlaceholder = document.getElementById('proc-canvas-placeholder');
const videoInput = document.getElementById('proc-video');
const uploadButtonLabel = document.getElementById('proc-upload-btn-label');
const selectedFileHint = document.getElementById('proc-selected-file-hint');
const togglePromptBtn = document.getElementById('proc-toggle-prompt');
const promptField = document.getElementById('proc-prompt');
const promptPresetSelect = document.getElementById('proc-prompt-preset');
const toggleVideoBg = document.getElementById('proc-toggle-video-bg');
const toggleInvertedMode = document.getElementById('proc-toggle-inverted-mode');
const backgroundImageBtn = document.getElementById('proc-background-image-btn');
const backgroundImageInput = document.getElementById('proc-background-image');
const toggleFace = document.getElementById('proc-toggle-face');
const toggleHand = document.getElementById('proc-toggle-hand');
const togglePose = document.getElementById('proc-toggle-pose');
const toggleObject = document.getElementById('proc-toggle-object');
const toggleGesture = document.getElementById('proc-toggle-gesture');
const toggleFaceDetect = document.getElementById('proc-toggle-face-detect');
const faceStyleSelect = document.getElementById('proc-face-style');
const overlayThicknessSlider = document.getElementById('proc-overlay-thickness');
const overlayThicknessValue = document.getElementById('proc-overlay-thickness-value');
const faceDotDensitySlider = document.getElementById('proc-face-dot-density');
const faceDotDensityValue = document.getElementById('proc-face-dot-density-value');
const togglePoseJoints = document.getElementById('proc-toggle-pose-joints');
const togglePoseTrails = document.getElementById('proc-toggle-pose-trails');
const toggleEmotionWheel = document.getElementById('toggle-emotion-wheel');
const playersPanel = document.querySelector('.proc-players-panel');
let landmarkCanvas = document.getElementById('proc-landmark-canvas');
const blendShapeList = document.getElementById('proc-blend-shape-list');
const emotionWheelContainer = document.getElementById('proc-emotion-wheel-container');
const circumplexSvgObject = document.getElementById('proc-circumplex-svg');
const tabData = document.getElementById('proc-tab-data');
const tabAi = document.getElementById('proc-tab-ai');
const viewData = document.getElementById('proc-view-data');
const viewAi = document.getElementById('proc-view-ai');
const captureFrameGroup = document.getElementById('proc-capture-frame-group');
const captureFrameBtn = document.getElementById('proc-capture-frame-btn');
const captureFrameMenuToggle = document.getElementById('proc-capture-frame-menu-toggle');
const captureFrameMenu = document.getElementById('proc-capture-frame-menu');
const captureFrameMenuCurrent = document.getElementById('proc-capture-frame-menu-current');
const captureFrameDestBtn = document.getElementById('proc-capture-frame-dest-btn');
const captureFrameDestClearBtn = document.getElementById('proc-capture-frame-dest-clear-btn');
const captureFramesetBtn = document.getElementById('proc-capture-frameset-btn');
...
const modeBar = document.getElementById('proc-mode-bar');
const libraryPanel = document.getElementById('proc-library-panel');
const libraryGrid = document.getElementById('proc-library-grid');
const archiveAnalyticsBtn = document.getElementById('proc-archive-analytics-btn');
const liveOverlay = document.getElementById('proc-live-overlay');
const liveSaveBtn = document.getElementById('proc-live-save-btn');
const liveStatus = document.getElementById('proc-live-status');
...
const imagePreviewEl = document.getElementById('proc-image-preview');
...
const updatePlayerOrientation = () => {
  ...
  playersPanel.classList.toggle('proc-vertical', isLandscape);
  playersPanel.classList.toggle('proc-horizontal', !isLandscape);
};
...
// ── Transport bar ──
const transportPlay = document.getElementById('proc-transport-play');
const transportPlayIcon = document.getElementById('proc-transport-play-icon');
const transportPauseIcon = document.getElementById('proc-transport-pause-icon');
const transportTimeline = document.getElementById('proc-transport-timeline');
const transportProgress = document.getElementById('proc-transport-progress');
const transportTime = document.getElementById('proc-transport-time');
...
document.getElementById('proc-landmark-canvas')?.addEventListener('click', () => {
...
const drawBlendShapesList = (blendShapes = []) => {
  ...
  const items = blendShapes[0].categories
    ...
    .map(
      (shape) => `
        <li class="proc-blend-shapes-item">
          <span class="proc-blend-shapes-label">${shape.displayName || shape.categoryName}</span>
          <span class="proc-blend-shapes-value">${(+shape.score).toFixed(4)}</span>
        </li>
      `
    )
    .join('');
...
const analyzeFaceFrame = () => {
  ...
  if (landmarkCanvas.classList.contains('proc-inverted-mode') !== invertActive) {
    landmarkCanvas.classList.toggle('proc-inverted-mode', invertActive);
  }
...
toggleInvertedMode?.addEventListener('change', (event) => {
  ...
  if (landmarkCanvas) {
    landmarkCanvas.classList.toggle('proc-inverted-mode', invertedModeEnabled && workspaceMode === 'edit');
  }
  markPreviewDirty();
});
...
const workspace = document.querySelector('.proc-workspace');
if (workspace) {
  workspace.classList.remove('proc-analytics-visible');
}
...
tabData?.addEventListener('click', () => {
  tabData.classList.add('proc-active');
  tabAi?.classList.remove('proc-active');
  if (viewData) viewData.hidden = false;
  if (viewAi) viewAi.hidden = true;
});

tabAi?.addEventListener('click', () => {
  tabAi.classList.add('proc-active');
  tabData?.classList.remove('proc-active');
...
const transportBar = document.getElementById('proc-transport-bar');
...
form.classList.add('proc-hidden');

// ── Loading screen / workspace entry ─────────────────────────────────────────

const workspaceRoot = document.getElementById('proc-workspace-root');
...
function showWorkspace() {
  ...
  form.classList.remove('proc-hidden');
  if (showAnalyticsBtn) {
  ...
  document.querySelector('.proc-workspace')?.classList.remove('proc-analytics-visible');
  appState = 'workspace';
...
function toggleAnalyticsPanel() {
  ...
  if (outputsPanel.hidden) {
    ...
    enableFaceLandmarks();
    if (tabData) tabData.classList.add('proc-active');
    if (tabAi) { tabAi.classList.remove('proc-active'); tabAi.hidden = true; }
    if (viewData) viewData.hidden = false;
    if (viewAi) viewAi.hidden = true;
    ...
    document.querySelector('.proc-workspace')?.classList.add('proc-analytics-visible');
  } else {
    closeAnalyticsPanel();
  }
}
...
function stopWebcam() {
  ...
  liveMode = false;
  transportBar?.classList.remove('proc-transport-bar--live');
}
...
function switchMode(mode) {
  ...
  if (mode === 'edit') {
    ...
    if (transportBar) {
      ...
      transportBar.classList.add('proc-transport-bar--live');
    }
  ...
  modeBar?.querySelectorAll('.proc-mode-btn').forEach((btn) => {
    btn.classList.toggle('proc-active', btn.dataset.mode === mode);
  });
}

modeBar?.addEventListener('click', (e) => {
  const btn = e.target.closest('.proc-mode-btn');
  if (!btn) return;
...
function renderLibrary(items) {
  ...
  items.forEach((item) => {
    ...
    div.className = 'proc-library-item';
    ...
    name.className = 'proc-library-item-name';
...
libraryGrid?.addEventListener('click', async (e) => {
  const item = e.target.closest('.proc-library-item');
  if (!item) return;
...
function openAnalyticsAiTab() {
  ...
  document.querySelector('.proc-workspace')?.classList.add('proc-analytics-visible');
  if (tabAi) { tabAi.hidden = false; tabAi.classList.add('proc-active'); }
  if (tabData) tabData.classList.remove('proc-active');
  if (viewAi) viewAi.hidden = false;
  if (viewData) viewData.hidden = true;
  if (showAnalyticsBtn) showAnalyticsBtn.textContent = 'Hide Analytics';
}
...
liveSaveBtn?.addEventListener('click', async () => {
  ...
  try {
    ...
    if (!aRes.ok || !aPayload?.resultText) {
      if (liveStatus) {
        liveStatus.innerHTML = `Saved! <a id="proc-open-archive-link">Open in Archive</a>`;
        document.getElementById('proc-open-archive-link')?.addEventListener('click', () => switchMode('archive'));
      }
      return;
    }
    ...
    openAnalyticsAiTab();
    if (liveStatus) {
      liveStatus.innerHTML = `Analysis ready. <a id="proc-open-archive-link">Open in Archive</a>`;
      document.getElementById('proc-open-archive-link')?.addEventListener('click', () => switchMode('archive'));
    }
  } catch (err) {
...
const loaderOverlay = document.getElementById('proc-loader-overlay');
...
const onDocClickForCaptureMenu = (e) => {
  ...
  if (e.target.closest('#proc-capture-frame-group')) return;
  closeCaptureMenu();
};
...
const framesetPopup = document.getElementById('proc-frameset-popup');
const framesetFrom = document.getElementById('proc-frameset-from');
const framesetTo = document.getElementById('proc-frameset-to');
const framesetDest = document.getElementById('proc-frameset-dest');
const framesetPrefix = document.getElementById('proc-frameset-prefix');
const framesetPickBtn = document.getElementById('proc-frameset-pick-btn');
const framesetWholeBtn = document.getElementById('proc-frameset-whole-btn');
const framesetCancelBtn = document.getElementById('proc-frameset-cancel-btn');
const framesetStartBtn = document.getElementById('proc-frameset-start-btn');
const exportOverlay = document.getElementById('proc-export-overlay');
const exportStatus = document.getElementById('proc-export-status');
const exportProgressBar = document.getElementById('proc-export-progress-bar');
const exportStopBtn = document.getElementById('proc-export-stop-btn');
...
const clearPickedDir = () => {
  ...
  if (framesetDest) {
    ...
    framesetDest.classList.remove('proc-picked');
  }
...
framesetPickBtn?.addEventListener('click', async () => {
  ...
  try {
    ...
    framesetDest.classList.add('proc-picked');
...
const aiControls = document.getElementById('proc-ai-controls');
const sendAnalysisBtn = document.getElementById('proc-send-analysis-btn');
...
const fullscreenOverlay = document.getElementById('proc-fullscreen-overlay');
const fullscreenContent = document.getElementById('proc-fullscreen-result-content');
const fullscreenOpenBtn = document.getElementById('proc-fullscreen-result-btn');
const fullscreenCloseBtn = document.getElementById('proc-fullscreen-close-btn');
