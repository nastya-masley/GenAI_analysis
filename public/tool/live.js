...
const form = document.getElementById('live-analyze-form');
const statusEl = document.getElementById('live-status');
const resultSection = document.getElementById('live-result');
const resultText = document.getElementById('live-result-text');
const submitBtn = document.getElementById('live-submit-btn');
const showAnalyticsBtn = document.getElementById('live-show-analytics-btn');
const outputsPanel = document.getElementById('live-analytics-bottom');
...
const canvasPlaceholder = document.getElementById('live-canvas-placeholder');
const videoInput = document.getElementById('live-video');
const uploadButtonLabel = document.getElementById('live-upload-btn-label');
const selectedFileHint = document.getElementById('live-selected-file-hint');
const togglePromptBtn = document.getElementById('live-toggle-prompt');
const promptField = document.getElementById('live-prompt');
const promptPresetSelect = document.getElementById('live-prompt-preset');
const toggleVideoBg = document.getElementById('live-toggle-video-bg');
const toggleInvertedMode = document.getElementById('live-toggle-inverted-mode');
const backgroundImageBtn = document.getElementById('live-background-image-btn');
const backgroundImageInput = document.getElementById('live-background-image');
const toggleFace = document.getElementById('live-toggle-face');
const toggleHand = document.getElementById('live-toggle-hand');
const togglePose = document.getElementById('live-toggle-pose');
const toggleObject = document.getElementById('live-toggle-object');
const toggleGesture = document.getElementById('live-toggle-gesture');
const toggleFaceDetect = document.getElementById('live-toggle-face-detect');
const faceStyleSelect = document.getElementById('live-face-style');
const overlayThicknessSlider = document.getElementById('live-overlay-thickness');
const overlayThicknessValue = document.getElementById('live-overlay-thickness-value');
const faceDotDensitySlider = document.getElementById('live-face-dot-density');
const faceDotDensityValue = document.getElementById('live-face-dot-density-value');
const togglePoseJoints = document.getElementById('live-toggle-pose-joints');
const togglePoseTrails = document.getElementById('live-toggle-pose-trails');
const toggleEmotionWheel = document.getElementById('toggle-emotion-wheel');
const playersPanel = document.querySelector('.live-players-panel');
let landmarkCanvas = document.getElementById('live-landmark-canvas');
const blendShapeList = document.getElementById('live-blend-shape-list');
const emotionWheelContainer = document.getElementById('live-emotion-wheel-container');
const circumplexSvgObject = document.getElementById('live-circumplex-svg');
const tabData = document.getElementById('live-tab-data');
const tabAi = document.getElementById('live-tab-ai');
const viewData = document.getElementById('live-view-data');
const viewAi = document.getElementById('live-view-ai');
const captureFrameGroup = document.getElementById('live-capture-frame-group');
const captureFrameBtn = document.getElementById('live-capture-frame-btn');
const captureFrameMenuToggle = document.getElementById('live-capture-frame-menu-toggle');
const captureFrameMenu = document.getElementById('live-capture-frame-menu');
const captureFrameMenuCurrent = document.getElementById('live-capture-frame-menu-current');
const captureFrameDestBtn = document.getElementById('live-capture-frame-dest-btn');
const captureFrameDestClearBtn = document.getElementById('live-capture-frame-dest-clear-btn');
const captureFramesetBtn = document.getElementById('live-capture-frameset-btn');
...
const libraryPanel = document.getElementById('live-library-panel');
const libraryGrid = document.getElementById('live-library-grid');
const archiveAnalyticsBtn = document.getElementById('live-archive-analytics-btn');
const liveOverlay = document.getElementById('live-live-overlay');
const liveSaveBtn = document.getElementById('live-live-save-btn');
const liveStatus = document.getElementById('live-live-status');
...
const imagePreviewEl = document.getElementById('live-image-preview');
...
const updatePlayerOrientation = () => {
  ...
  playersPanel.classList.toggle('live-vertical', isLandscape);
  playersPanel.classList.toggle('live-horizontal', !isLandscape);
};
...
// ── Transport bar ──
const transportPlay = document.getElementById('live-transport-play');
const transportPlayIcon = document.getElementById('live-transport-play-icon');
const transportPauseIcon = document.getElementById('live-transport-pause-icon');
const transportTimeline = document.getElementById('live-transport-timeline');
const transportProgress = document.getElementById('live-transport-progress');
const transportTime = document.getElementById('live-transport-time');
...
document.getElementById('live-landmark-canvas')?.addEventListener('click', () => {
...
const drawBlendShapesList = (blendShapes = []) => {
  ...
  const items = blendShapes[0].categories
    ...
    .map(
      (shape) => `
        <li class="live-blend-shapes-item">
          <span class="live-blend-shapes-label">${shape.displayName || shape.categoryName}</span>
          <span class="live-blend-shapes-value">${(+shape.score).toFixed(4)}</span>
        </li>
      `
    )
    .join('');
...
const analyzeFaceFrame = () => {
  ...
  if (landmarkCanvas.classList.contains('live-inverted-mode') !== invertActive) {
    landmarkCanvas.classList.toggle('live-inverted-mode', invertActive);
  }
...
toggleInvertedMode?.addEventListener('change', (event) => {
  ...
  if (landmarkCanvas) {
    landmarkCanvas.classList.toggle('live-inverted-mode', invertedModeEnabled && workspaceMode === 'edit');
  }
  markPreviewDirty();
});
...
if (workspace) {
  workspace.classList.remove('live-analytics-visible');
}
...
tabData?.addEventListener('click', () => {
  tabData.classList.add('live-active');
  tabAi?.classList.remove('live-active');
  if (viewData) viewData.hidden = false;
  if (viewAi) viewAi.hidden = true;
});

tabAi?.addEventListener('click', () => {
  tabAi.classList.add('live-active');
  tabData?.classList.remove('live-active');
...
const transportBar = document.getElementById('live-transport-bar');
...
form.classList.add('live-hidden');

// ── Loading screen / workspace entry ─────────────────────────────────────────

const workspaceRoot = document.getElementById('live-workspace-root');
...
function showWorkspace() {
  ...
  form.classList.remove('live-hidden');
  if (showAnalyticsBtn) {
  ...
  document.querySelector('.live-workspace')?.classList.remove('live-analytics-visible');
  appState = 'workspace';
...
function toggleAnalyticsPanel() {
  ...
  if (outputsPanel.hidden) {
    ...
    enableFaceLandmarks();
    if (tabData) tabData.classList.add('live-active');
    if (tabAi) { tabAi.classList.remove('live-active'); tabAi.hidden = true; }
    if (viewData) viewData.hidden = false;
    if (viewAi) viewAi.hidden = true;
    ...
    document.querySelector('.live-workspace')?.classList.add('live-analytics-visible');
  } else {
    closeAnalyticsPanel();
  }
}
...
function stopWebcam() {
  ...
  liveMode = false;
  transportBar?.classList.remove('live-transport-bar--live');
}
...
function switchMode(mode) {
  ...
  if (mode === 'edit') {
    ...
    if (transportBar) {
      ...
      transportBar.classList.add('live-transport-bar--live');
    }
  ...
  modeBar?.querySelectorAll('.live-mode-btn').forEach((btn) => {
    btn.classList.toggle('live-active', btn.dataset.mode === mode);
  });
}

modeBar?.addEventListener('click', (e) => {
  const btn = e.target.closest('.live-mode-btn');
  if (!btn) return;
...
function renderLibrary(items) {
  ...
  items.forEach((item) => {
    ...
    div.className = 'live-library-item';
    ...
    name.className = 'live-library-item-name';
...
libraryGrid?.addEventListener('click', async (e) => {
  const item = e.target.closest('.live-library-item');
  if (!item) return;
...
function openAnalyticsAiTab() {
  ...
  document.querySelector('.live-workspace')?.classList.add('live-analytics-visible');
  if (tabAi) { tabAi.hidden = false; tabAi.classList.add('live-active'); }
  if (tabData) tabData.classList.remove('live-active');
  if (viewAi) viewAi.hidden = false;
  if (viewData) viewData.hidden = true;
  if (showAnalyticsBtn) showAnalyticsBtn.textContent = 'Hide Analytics';
}
...
liveSaveBtn?.addEventListener('click', async () => {
  ...
  try {
    ...
    if (!aRes.ok || !aPayload?.resultText) {
      if (liveStatus) {
        liveStatus.innerHTML = `Saved! <a id="live-open-archive-link">Open in Archive</a>`;
        document.getElementById('live-open-archive-link')?.addEventListener('click', () => (window.__toolGoMode && window.__toolGoMode('archive')));
      }
      return;
    }
    ...
    openAnalyticsAiTab();
    if (liveStatus) {
      liveStatus.innerHTML = `Analysis ready. <a id="live-open-archive-link">Open in Archive</a>`;
      document.getElementById('live-open-archive-link')?.addEventListener('click', () => (window.__toolGoMode && window.__toolGoMode('archive')));
    }
  } catch (err) {
...
const loaderOverlay = document.getElementById('live-loader-overlay');
...
const onDocClickForCaptureMenu = (e) => {
  ...
  if (e.target.closest('#live-capture-frame-group')) return;
  closeCaptureMenu();
};
...
const framesetPopup = document.getElementById('live-frameset-popup');
const framesetFrom = document.getElementById('live-frameset-from');
const framesetTo = document.getElementById('live-frameset-to');
const framesetDest = document.getElementById('live-frameset-dest');
const framesetPrefix = document.getElementById('live-frameset-prefix');
const framesetPickBtn = document.getElementById('live-frameset-pick-btn');
const framesetWholeBtn = document.getElementById('live-frameset-whole-btn');
const framesetCancelBtn = document.getElementById('live-frameset-cancel-btn');
const framesetStartBtn = document.getElementById('live-frameset-start-btn');
const exportOverlay = document.getElementById('live-export-overlay');
const exportStatus = document.getElementById('live-export-status');
const exportProgressBar = document.getElementById('live-export-progress-bar');
const exportStopBtn = document.getElementById('live-export-stop-btn');
...
const clearPickedDir = () => {
  ...
  if (framesetDest) {
    ...
    framesetDest.classList.remove('live-picked');
  }
...
framesetPickBtn?.addEventListener('click', async () => {
  ...
  try {
    ...
    framesetDest.classList.add('live-picked');
...
const aiControls = document.getElementById('live-ai-controls');
const sendAnalysisBtn = document.getElementById('live-send-analysis-btn');
...
const fullscreenOverlay = document.getElementById('live-fullscreen-overlay');
const fullscreenContent = document.getElementById('live-fullscreen-result-content');
const fullscreenOpenBtn = document.getElementById('live-fullscreen-result-btn');
const fullscreenCloseBtn = document.getElementById('live-fullscreen-close-btn');
