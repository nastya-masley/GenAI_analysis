...
const form = document.getElementById('arch-analyze-form');
const statusEl = document.getElementById('arch-status');
const resultSection = document.getElementById('arch-result');
const resultText = document.getElementById('arch-result-text');
const submitBtn = document.getElementById('arch-submit-btn');
const showAnalyticsBtn = document.getElementById('arch-show-analytics-btn');
const outputsPanel = document.getElementById('arch-analytics-bottom');
const previewEl = document.getElementById('arch-preview');
const videoPlaceholder = document.getElementById('arch-video-placeholder');
const canvasPlaceholder = document.getElementById('arch-canvas-placeholder');
const videoInput = document.getElementById('arch-video');
const uploadButtonLabel = document.getElementById('arch-upload-btn-label');
const selectedFileHint = document.getElementById('arch-selected-file-hint');
const togglePromptBtn = document.getElementById('arch-toggle-prompt');
const promptField = document.getElementById('arch-prompt');
const promptPresetSelect = document.getElementById('arch-prompt-preset');
const toggleVideoBg = document.getElementById('arch-toggle-video-bg');
const toggleInvertedMode = document.getElementById('arch-toggle-inverted-mode');
const backgroundImageBtn = document.getElementById('arch-background-image-btn');
const backgroundImageInput = document.getElementById('arch-background-image');
const toggleFace = document.getElementById('arch-toggle-face');
const toggleHand = document.getElementById('arch-toggle-hand');
const togglePose = document.getElementById('arch-toggle-pose');
const toggleObject = document.getElementById('arch-toggle-object');
const toggleGesture = document.getElementById('arch-toggle-gesture');
const toggleFaceDetect = document.getElementById('arch-toggle-face-detect');
const faceStyleSelect = document.getElementById('arch-face-style');
const overlayThicknessSlider = document.getElementById('arch-overlay-thickness');
const overlayThicknessValue = document.getElementById('arch-overlay-thickness-value');
const faceDotDensitySlider = document.getElementById('arch-face-dot-density');
const faceDotDensityValue = document.getElementById('arch-face-dot-density-value');
const togglePoseJoints = document.getElementById('arch-toggle-pose-joints');
const togglePoseTrails = document.getElementById('arch-toggle-pose-trails');
const toggleEmotionWheel = document.getElementById('toggle-emotion-wheel');
const playersPanel = document.querySelector('.arch-players-panel');
let landmarkCanvas = document.getElementById('arch-landmark-canvas');
const blendShapeList = document.getElementById('arch-blend-shape-list');
const emotionWheelContainer = document.getElementById('arch-emotion-wheel-container');
const circumplexSvgObject = document.getElementById('arch-circumplex-svg');
const tabData = document.getElementById('arch-tab-data');
const tabAi = document.getElementById('arch-tab-ai');
const viewData = document.getElementById('arch-view-data');
const viewAi = document.getElementById('arch-view-ai');
const captureFrameGroup = document.getElementById('arch-capture-frame-group');
const captureFrameBtn = document.getElementById('arch-capture-frame-btn');
const captureFrameMenuToggle = document.getElementById('arch-capture-frame-menu-toggle');
const captureFrameMenu = document.getElementById('arch-capture-frame-menu');
const captureFrameMenuCurrent = document.getElementById('arch-capture-frame-menu-current');
const captureFrameDestBtn = document.getElementById('arch-capture-frame-dest-btn');
const captureFrameDestClearBtn = document.getElementById('arch-capture-frame-dest-clear-btn');
const captureFramesetBtn = document.getElementById('arch-capture-frameset-btn');
...
const modeBar = document.getElementById('arch-mode-bar');
const libraryPanel = document.getElementById('arch-library-panel');
const libraryGrid = document.getElementById('arch-library-grid');
const archiveAnalyticsBtn = document.getElementById('arch-archive-analytics-btn');
const liveOverlay = document.getElementById('arch-live-overlay');
const liveSaveBtn = document.getElementById('arch-live-save-btn');
const liveStatus = document.getElementById('arch-live-status');
...
const imagePreviewEl = document.getElementById('arch-image-preview');
...
const updatePlayerOrientation = () => {
  ...
  playersPanel.classList.toggle('arch-vertical', isLandscape);
  playersPanel.classList.toggle('arch-horizontal', !isLandscape);
};
...
// ── Transport bar ──
const transportPlay = document.getElementById('arch-transport-play');
const transportPlayIcon = document.getElementById('arch-transport-play-icon');
const transportPauseIcon = document.getElementById('arch-transport-pause-icon');
const transportTimeline = document.getElementById('arch-transport-timeline');
const transportProgress = document.getElementById('arch-transport-progress');
const transportTime = document.getElementById('arch-transport-time');
...
document.getElementById('arch-landmark-canvas')?.addEventListener('click', () => {
...
const drawBlendShapesList = (blendShapes = []) => {
  ...
  const items = blendShapes[0].categories
    ...
    .map(
      (shape) => `
        <li class="arch-blend-shapes-item">
          <span class="arch-blend-shapes-label">${shape.displayName || shape.categoryName}</span>
          <span class="arch-blend-shapes-value">${(+shape.score).toFixed(4)}</span>
        </li>
      `
    )
    .join('');
...
const analyzeFaceFrame = () => {
  ...
  if (landmarkCanvas.classList.contains('arch-inverted-mode') !== invertActive) {
    landmarkCanvas.classList.toggle('arch-inverted-mode', invertActive);
  }
...
toggleInvertedMode?.addEventListener('change', (event) => {
  ...
  if (landmarkCanvas) {
    landmarkCanvas.classList.toggle('arch-inverted-mode', invertedModeEnabled && workspaceMode === 'edit');
  }
  markPreviewDirty();
});
...
if (workspace) {
  workspace.classList.remove('arch-analytics-visible');
}
...
tabData?.addEventListener('click', () => {
  tabData.classList.add('arch-active');
  tabAi?.classList.remove('arch-active');
  if (viewData) viewData.hidden = false;
  if (viewAi) viewAi.hidden = true;
});

tabAi?.addEventListener('click', () => {
  tabAi.classList.add('arch-active');
  tabData?.classList.remove('arch-active');
...
const transportBar = document.getElementById('arch-transport-bar');
...
form.classList.add('arch-hidden');

// ── Loading screen / workspace entry ─────────────────────────────────────────

const workspaceRoot = document.getElementById('arch-workspace-root');
...
function showWorkspace() {
  ...
  form.classList.remove('arch-hidden');
  if (showAnalyticsBtn) {
  ...
  document.querySelector('.arch-workspace')?.classList.remove('arch-analytics-visible');
  appState = 'workspace';
...
function toggleAnalyticsPanel() {
  ...
  if (outputsPanel.hidden) {
    ...
    enableFaceLandmarks();
    if (tabData) tabData.classList.add('arch-active');
    if (tabAi) { tabAi.classList.remove('arch-active'); tabAi.hidden = true; }
    if (viewData) viewData.hidden = false;
    if (viewAi) viewAi.hidden = true;
    ...
    document.querySelector('.arch-workspace')?.classList.add('arch-analytics-visible');
  } else {
    closeAnalyticsPanel();
  }
}
...
function stopWebcam() {
  ...
  liveMode = false;
  transportBar?.classList.remove('arch-transport-bar--live');
}
...
function switchMode(mode) {
  ...
  if (mode === 'edit') {
    ...
    if (transportBar) {
      ...
      transportBar.classList.add('arch-transport-bar--live');
    }
  ...
  modeBar?.querySelectorAll('.arch-mode-btn').forEach((btn) => {
    btn.classList.toggle('arch-active', btn.dataset.mode === mode);
  });
}

modeBar?.addEventListener('click', (e) => {
  const btn = e.target.closest('.arch-mode-btn');
  if (!btn) return;
...
function renderLibrary(items) {
  ...
  items.forEach((item) => {
    ...
    div.className = 'arch-library-item';
    ...
    name.className = 'arch-library-item-name';
...
libraryGrid?.addEventListener('click', async (e) => {
  const item = e.target.closest('.arch-library-item');
  if (!item) return;
...
function openAnalyticsAiTab() {
  ...
  document.querySelector('.arch-workspace')?.classList.add('arch-analytics-visible');
  if (tabAi) { tabAi.hidden = false; tabAi.classList.add('arch-active'); }
  if (tabData) tabData.classList.remove('arch-active');
  if (viewAi) viewAi.hidden = false;
  if (viewData) viewData.hidden = true;
  if (showAnalyticsBtn) showAnalyticsBtn.textContent = 'Hide Analytics';
}
...
liveSaveBtn?.addEventListener('click', async () => {
  ...
  try {
    ...
    if (!aRes.ok || !aPayload?.resultText) {
      if (liveStatus) {
        liveStatus.innerHTML = `Saved! <a id="arch-open-archive-link">Open in Archive</a>`;
        document.getElementById('arch-open-archive-link')?.addEventListener('click', () => switchMode('archive'));
      }
      return;
    }
    ...
    openAnalyticsAiTab();
    if (liveStatus) {
      liveStatus.innerHTML = `Analysis ready. <a id="arch-open-archive-link">Open in Archive</a>`;
      document.getElementById('arch-open-archive-link')?.addEventListener('click', () => switchMode('archive'));
    }
  } catch (err) {
...
const loaderOverlay = document.getElementById('arch-loader-overlay');
...
const onDocClickForCaptureMenu = (e) => {
  ...
  if (e.target.closest('#arch-capture-frame-group')) return;
  closeCaptureMenu();
};
...
const framesetPopup = document.getElementById('arch-frameset-popup');
const framesetFrom = document.getElementById('arch-frameset-from');
const framesetTo = document.getElementById('arch-frameset-to');
const framesetDest = document.getElementById('arch-frameset-dest');
const framesetPrefix = document.getElementById('arch-frameset-prefix');
const framesetPickBtn = document.getElementById('arch-frameset-pick-btn');
const framesetWholeBtn = document.getElementById('arch-frameset-whole-btn');
const framesetCancelBtn = document.getElementById('arch-frameset-cancel-btn');
const framesetStartBtn = document.getElementById('arch-frameset-start-btn');
const exportOverlay = document.getElementById('arch-export-overlay');
const exportStatus = document.getElementById('arch-export-status');
const exportProgressBar = document.getElementById('arch-export-progress-bar');
const exportStopBtn = document.getElementById('arch-export-stop-btn');
...
const clearPickedDir = () => {
  ...
  if (framesetDest) {
    ...
    framesetDest.classList.remove('arch-picked');
  }
...
framesetPickBtn?.addEventListener('click', async () => {
  ...
  try {
    ...
    framesetDest.classList.add('arch-picked');
...
const aiControls = document.getElementById('arch-ai-controls');
const sendAnalysisBtn = document.getElementById('arch-send-analysis-btn');
...
const fullscreenOverlay = document.getElementById('arch-fullscreen-overlay');
const fullscreenContent = document.getElementById('arch-fullscreen-result-content');
const fullscreenOpenBtn = document.getElementById('arch-fullscreen-result-btn');
const fullscreenCloseBtn = document.getElementById('arch-fullscreen-close-btn');
