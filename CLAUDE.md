...
1. Client sends video + prompt to `POST /api/analyze`. Every TYPE preset's `prompt` has `RESPONSE_STYLE_RULES` appended (no semicolons; capitalize the first letter of each line) **except TYPE_02** (`pose-rows`) and TYPE_05 (`ask-aema`, which bakes the same style rule into its own format line). **Language level (CEFR B2–C1)**: the factual TYPEs **TYPE_01** + **TYPE_03** also get `LANGUAGE_LEVEL_RULE` appended (all prose readable at B2–C1, ≤ C1; scores/labels/codes untouched); the poetic **TYPE_04** gets `LANGUAGE_LEVEL_RULE_POETIC` (keep the literary voice but simple, common words ≤ C1); **TYPE_05**'s `Response format:` line carries the same B2–C1 constraint inline (covers both the relevant answer and the poetic-scare). **TYPE_02** is left unconstrained (its single-word pose output is by design).
