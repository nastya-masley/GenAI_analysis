...
- **Rolling 15-second buffer** (`LIVE_WINDOW_MS = 15000`). `MediaRecorder` records the **mirrored** webcam (no overlay) in 1s chunks; `rotateCacheRecording()` rotates each window so a finalized previous blob is always available. See **Mirrored (selfie) view**.
