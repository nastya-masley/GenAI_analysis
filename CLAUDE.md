...
4. Client: `renderAnalysisResult(text, activePreset)` dispatches on the active preset's `format` field (captured at request-send time so a mid-flight selection change can't repaint with the wrong format):
   ...
   - Both custom-format classes (`.pose-output`, `.facs-output`) are cleared on every render — including the default markdown branch and the error/`geminiResponse` branch — so styling from one format never leaks into the next response.
