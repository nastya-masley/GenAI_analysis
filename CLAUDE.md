...
- Unchecked → `showVideoBackground = false`, reveals the background sub-list (`#video-bg-custom-btns`). The single `backgroundImage` slot (null = blank, AEMA logo, or a custom image) is drawn instead of video; null + video-off = black canvas with overlays only ([public/script.js](public/script.js) render branch in `analyzeFaceFrame`). **On switching video bg off, AEMA defaults ON** when nothing else is set (`!backgroundImage` → checks `#toggle-aema-bg` + loads the AEMA logo), so video-off never lands on a bare black canvas by default; a previously-chosen custom image (non-null `backgroundImage`) is preserved instead.
- Sub-list items (order):
  ...
  2. **Choose background** (`#background-image-btn`, a `.toggle` wrapping a hidden `#background-image` file input + a decorative `#toggle-custom-bg` checkbox) — loads a custom still (stretched); unchecks AEMA (custom replaces it).
