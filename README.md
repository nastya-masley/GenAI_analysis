# Nonverbal AI.COM Analysis

Simple Express + vanilla JS app that lets you upload or record a short video, send it to Nonverbal AI.COM with a customizable prompt, and view the generated response.

## Requirements
- Node.js 18+
- Google Gemini API access + key (used under the hood by Nonverbal AI.COM)
...
Use the form to either upload a file (<=`MAX_VIDEO_SIZE_MB`, 200 MB by default) or enable the webcam to record a new clip. The app automatically sends a built-in Nonverbal AI.COM prompt; if you need to tweak it, click **Customize prompt** to reveal and edit the full template before submitting. Results appear once Nonverbal AI.COM finishes analysis.
...
- All MediaPipe processing happens locally in the browser and does not influence the Nonverbal AI.COM upload.
...
- Nonverbal AI.COM errors / timeouts → check server logs for the response payload in the terminal.
