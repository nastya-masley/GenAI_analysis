...
Simple Express + vanilla JS app that lets you upload or record a short video, send it to AI with a customizable prompt, and view the generated response.
...
Use the form to either upload a file (<=`MAX_VIDEO_SIZE_MB`, 200 MB by default) or enable the webcam to record a new clip. The app automatically sends a built-in AI prompt; if you need to tweak it, click **Customize prompt** to reveal and edit the full template before submitting. Results appear once AI finishes analysis.
